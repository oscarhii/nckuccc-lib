...
$t=$t.Replace("<div className=\"grouped\">{Object.entries(grouped).map", "<div className=\"dictionary-index\"><button className={!group?'on':''} onClick={()=>setGroup('')}>全部</button>{Object.keys(grouped).map(g=><button className={group===g?'on':''} onClick={()=>setGroup(g)}>{g}</button>)}</div><div className=\"grouped\">{Object.entries(grouped).filter(([g])=>!group||g===group).map")
